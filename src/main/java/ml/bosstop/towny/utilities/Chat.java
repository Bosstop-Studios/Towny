package ml.bosstop.towny.utilities;
...
public class Chat {
    ...
    private final String prefix = "&0[&bPR&0]&r";
    ...
    public void sendRaw(CommandSender sender, String input) {
    ...
    public void console(String input) {
        System.out.println(ANSIIConverter(translateCharacter, input));
    }
    ...
    private String ANSIIConverter(char altColorChar, String text) {
        for (Entry<String, String> entry : MCToANSII.entrySet()) {
            text = text.replaceAll(altColorChar + entry.getKey(), entry.getValue());
        }
        return text + MCToANSII.get("r");
    }
